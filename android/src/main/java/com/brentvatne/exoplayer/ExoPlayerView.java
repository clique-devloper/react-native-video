...
public final class ExoPlayerView extends FrameLayout implements AdViewProvider {
    ...
    // 자막 위치 퍼센테이지 (0~100, 0=상단, 100=하단)
    private int subtitlePaddingBottom = 85;
    ...
    public void setSubtitleStyle(SubtitleStyle style) {
        ...
        // paddingBottom을 퍼센테이지로 저장 (0~100)
        subtitlePaddingBottom = style.getPaddingBottom();
        Log.d("ExoPlayerView", "자막 위치 설정: " + subtitlePaddingBottom + "%");
        
        subtitleLayout.setPadding(style.getPaddingLeft(), style.getPaddingTop(), style.getPaddingRight(), 0);
    ...
    private final class ComponentListener implements Player.Listener {
        ...
        public void onCues(@NonNull List<Cue> cues) {
            // 자막 위치를 퍼센테이지 기반으로 강제 조정
            if (cues != null && !cues.isEmpty()) {
                List<Cue> modifiedCues = new ArrayList<>();
                
                for (Cue cue : cues) {
                    // paddingBottom 퍼센테이지를 line 값으로 변환 (0~100 -> 0.0~1.0)
                    float linePosition = subtitlePaddingBottom / 100.0f;
                    
                    Cue modifiedCue = cue.buildUpon()
                ...
                Log.d("ExoPlayerView", "자막 위치 적용: " + subtitlePaddingBottom + "% -> line=" + (subtitlePaddingBottom / 100.0f));
                subtitleLayout.setCues(modifiedCues);
            } else {
